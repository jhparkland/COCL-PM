...
class CPUID(object):
    def __init__(self):
        ...
        # Use getauxval to get hardware capabilities
            try:
                # Run the system_profiler command to get the serial number
                result = subprocess.run(["system_profiler", "SPHardwareDataType"], stdout=subprocess.PIPE, text=True)

                # Search for the serial number in the result
                serial_number_line = next(line for line in result.stdout.splitlines() if "Serial Number" in line)
                self.serial_number = serial_number_line.split(":")[1].strip()
                # print(serial_number)
